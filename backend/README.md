...
Проект поддерживает два варианта базы данных и автоматически выбирает, что использовать, на основе переменных в `.env`:

1. **SQLite (по умолчанию)** – если не указан `DATABASE_URL` **и** не заданы параметры PostgreSQL.
2. **PostgreSQL** – если указан `DATABASE_URL` **или** заданы все параметры `DB_HOST`, `DB_USER`, `DB_NAME`.

### SQLite (по умолчанию, для тестов/разработки)

Если в `.env` ничего не указывать для БД (оставить `DATABASE_URL` пустым и не заполнять `DB_HOST/DB_USER/DB_NAME`), будет использован SQLite:

- Файл БД создаётся автоматически по пути `./db/bot.db` (относительно директории `backend`).
- Подходит для локальной разработки, тестов и небольших инсталляций.
...
Если `DATABASE_URL` непустой – он используется напрямую.

#### Вариант 2: Отдельные параметры (рекомендуется для безопасности)

Если `DATABASE_URL` **пустой**, но заданы все три параметра `DB_HOST`, `DB_USER`, `DB_NAME`, URL собирается автоматически:
...
В этом случае приложение сформирует строку подключения вида:
