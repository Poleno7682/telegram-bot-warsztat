...
- [x] 6.1 Настроить `structlog` (JSON-формат, contextvars), заменить текущие `print`/logging-вызовы.
- [x] 6.2 Добавить базовые метрики (счётчики напоминаний, переводов) и health-check команду.

## 7. Инфраструктура
- [x] 7.1 Подготовить `systemd` unit + `logrotate` конфиг + инструкции для VPS (в README).
- [x] 7.2 Обновить документацию (.env.example, шаги деплоя, требования к серверу).

## 8. Тестирование
- [x] 8.1 Добавить модульные тесты для новых сервисов/переводов/напоминаний.
- [x] 8.2 Автоматизировать проверку миграций (скрипт или CI-шаг `alembic upgrade --sql`).
