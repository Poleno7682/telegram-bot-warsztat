...
| 2.4.5 | разные админ-хендлеры | ~~`callback.answer()` используется напрямую вместо общего `safe_callback_answer()` (`admin/services.py`, `admin/settings.py`, `user_settings.py`, `calendar.py`, `start.py:78`)~~ | ✅ Исправлено 2026-07-23 — все ~30 вызовов `callback.answer(...)` в этих 5 файлах заменены на `safe_callback_answer(callback, ...)`, добавлены недостающие импорты |
| 2.4.6 | `admin/settings.py` (work-hours/time-step/buffer handlers) | ~~`except ValueError` — не-ValueError исключение (например, ошибка БД) оставляет FSM в состоянии ожидания ввода "навсегда"~~ | ✅ Исправлено 2026-07-23 — `work_end_entered`/`time_step_entered`/`buffer_time_entered` разделены на парсинг (только `ValueError`, даёт повторить ввод) и вызов сервиса (`except Exception` + `finally: state.clear()` + возврат в меню); `work_start_entered` не тронут — там нет вызова сервиса/БД, реального риска не было |
| 2.4.7 | `user_settings.py:50` | ~~Захардкожена двуязычная PL/RU строка вместо i18n~~ | ✅ Исправлено 2026-07-23 — `get_language_display_name` принимает `translate` и использует уже существующий ключ `user_settings.language_not_set` (тот же, что уже использовался напрямую в одном из двух call site — ветка с хардкодом была фактически мёртвым кодом, недостижимым при текущих вызовах) |
| 2.4.8 | `admin/services.py:248` | ~~При ошибке `create_service` — тишина, состояние просто сбрасывается~~ | ✅ Исправлено 2026-07-23 — вызов обёрнут в `try/except Exception` (лог + `errors.unknown` пользователю) и добавлен `else` для теоретического `service is None`; `finally` гарантирует `state.clear()` + возврат в меню в любом исходе |

---

**Этап 2.4 полностью закрыт** (2026-07-23): все восемь точечных проблем
(2.4.1–2.4.8) исправлены, каждая — отдельным коммитом с тестом. Это
завершает весь план `REFACTORING_PLAN_2026-07.md` — пункты 0.1–0.5, 1.1–1.4,
2.1–2.4 все отмечены как исправленные (2.1 и 2.3 — частично, по объективным
причинам, описанным в соответствующих секциях). Полный набор тестов:
117/117 проходит.
