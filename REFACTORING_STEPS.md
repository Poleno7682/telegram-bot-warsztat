...
- [ ] 2.2 Добавить простую фабрику/контейнер для выдачи репозиториев/сервисов в хэндлеры (чтобы не создавать объекты вручную в каждом handler).

## 3. Работа с переводами
- [ ] 3.1 Переписать `TranslationService` на класс с ограниченным кешем (LRU или Redis) и стратегией fallback.
- [ ] 3.2 Добавить обработку таймаутов/исключений и конфигурируемый список целевых языков.

## 4. Напоминания и хранение времени
- [ ] 4.1 Убедиться, что все `booking_date` сохраняются в UTC: преобразование при вводе + миграция/валидация данных.
- [ ] 4.2 Оптимизировать `get_bookings_for_reminders`: SQL-фильтр по `reminder_*_sent`, индексы на `booking_date`, лимит выборки.
- [ ] 4.3 Обернуть `ReminderScheduler` в контекстный менеджер/службу с гарантированным `stop()` и обработкой сбоев.

## 5. UX и меню
- [ ] 5.1 Реализовать менеджер отложенных сообщений (дедупликация отправки главного меню на чат).
- [ ] 5.2 Добавить троттлинг для сервисных уведомлений (например, при ошибках перевода).
